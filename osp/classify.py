...
def classify_data(data, target_col='_target', cv=10, verbose=True, balance=False, normalize=False, sample_size=None, **kwargs):
    ...
    if balance:
        ...
        sample_size = min_target_size if sample_size is None or sample_size > min_target_size else sample_size
...
def iter_pairwise_samples(data, colname='period', data_meta=None, balance=True, sample_size=None):
    ...
    for coltype1 in coltypes:
        for coltype2 in coltypes:
            ...
            data_smpl_yes = data_smpl2.query(f'{index_name} in @ok_yes').assign(_target=coltype1)
            data_smpl_no = data_smpl2.query(f'{index_name} in @ok_no').assign(_target=coltype2)
...
def classify_by_feat_counts(groups, predict_all=False, num_runs=1, sample_size=None, incl_deprel=True, incl_pos=True, cv=10, feat_n=10, feat_min_count=1, avg_runs=True, balance=True, **kwargs):
    ...
    if l_predicts:
        ...
        odf_preds = pd.concat([odf_preds.assign(predict_type='cv'), odf_predicts.assign(predict_type='unseen')])
...
def classify_then_predict(groups_train, target_col='discipline', balance=True, num_runs=1, verbose=False, **kwargs):
    ...
    for nrun in tqdm(list(range(num_runs))):
        df_scores = get_balanced_cv_data(groups_train, target_col=target_col, balance=balance, **kwargs)
        ...
        df_out_probs = pd.concat([cv_preds.assign(run=nrun, predict_type='cv'), df_new_probs.assign(run=nrun, predict_type='unseen')])
...
def classify_then_predict_comparisons(comparison, **kwargs):
    l_preds = []
    l_feats = []
    for groups_train in comparison:
        comparison_name = f'{groups_train[0][0]} vs {groups_train[1][0]}'
        print('##',comparison_name)
        df_preds, df_feats = classify_then_predict(groups_train, **kwargs)
    ...
    odf_feats['score_mean_diff_pct'] = odf_feats['score_mean_diff'] / odf_feats['score_mean2']
    ...
    return odf_preds, odf_feats.sort_values('weight', ascending=False)
